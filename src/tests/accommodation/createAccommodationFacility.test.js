...
import app from '../../app';
import {
  facility, wrongUser, travelAdmin,
  existingFacility,
  missingInformation,
  missingRoomInfo,
  editAccommodation,
    ...
    it('it should return 400  when information is missing', (done) => {
      chai
        ...
        .end((err, res) => {
          ...
          expect(res.body.error).to.equal('"roomType" is required');
          done();
        });
    });
  });
...
export const getSingleAccommodation = () => {
  describe('accommodation ', () => {
    ...
    it('it should return 404 when there is no accommodation found ', (done) => {
      chai
        .request(app)
        .get(`/api/v1/view/accommodation/${7}`)
        .end((err, res) => {
          expect(res.statusCode).to.equal(404);
          expect(res.body.errorMessage).to.equal('accommmodation not found');
        });
      done();
    });
  });
