...
export default class accomodationFacility {
 ...
 const singleAccommodation = await models.Accommodation.findOne({where:{id:accommodationId}});
 if(singleAccommodation === null) return res.status(404).json({status:404, errorMessage:'accommmodation not found'});
...
if(Object.keys(req.body).length === 0) return res.status(400).json({status:400, errorMessage:'You are sending with empty fields'})
 models.Accommodation.update( req.body ,{where:{id:req.params.id}})
 .then(()=> models.Accommodation.findOne({where:{id:req.params.id}})
 .then((accommodation)=>{
  ...
  static  uploadBuildingImage(req,res){
    ...
    models.Accommodation.update(req.body,{where:{id:req.params.id}}).then(()=> models.Accommodation.findOne({where:{id:req.params.id}}))
    .then((accommodation)=>{
    if( accommodation === null) return res.status(404).json({status:404, errorMessage:'Accommodation not found'})
